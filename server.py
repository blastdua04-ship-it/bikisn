...
# On generic PaaS/container hosts there may be no convenient secret-setting UI.
# Persist the generated password on the /data volume so a container restart
# does not silently change the login credentials. An explicit ADMIN_PASSWORD
# always wins.
if not ADMIN_PASSWORD:
    _admin_password_file = Path(HERMES_HOME) / ".admin_password"
    try:
        ADMIN_PASSWORD = _admin_password_file.read_text().strip()
    except OSError:
        ADMIN_PASSWORD = ""
    if not ADMIN_PASSWORD:
        ...
        try:
            ...
            _admin_password_file.write_text(ADMIN_PASSWORD + "\n")
            _admin_password_file.chmod(0o600)
        except OSError as e:
            print(f"[server] could not persist generated admin password: {e!r}", flush=True)
...
def hermes_dashboard_public_url() -> str:
    """Return the public origin used for Hermes OAuth redirects.

    Railway exposes RAILWAY_PUBLIC_DOMAIN, but non-Railway hosts (including
    Deplexo) generally expose a normal URL instead. An explicit
    HERMES_DASHBOARD_PUBLIC_URL always wins; PUBLIC_URL/APP_URL are convenient
    fallbacks for generic PaaS deployments.
    """
    explicit = os.environ.get("HERMES_DASHBOARD_PUBLIC_URL", "").strip()
    if explicit:
        return explicit.rstrip("/")
    for key in ("PUBLIC_URL", "APP_URL"):
...
_cookie_secret_file = Path(HERMES_HOME) / ".admin_cookie_secret"
try:
    COOKIE_SECRET = bytes.fromhex(_cookie_secret_file.read_text().strip())
    if len(COOKIE_SECRET) != 32:
        raise ValueError("invalid cookie secret length")
except Exception:
    COOKIE_SECRET = secrets.token_bytes(32)
    try:
        _cookie_secret_file.parent.mkdir(parents=True, exist_ok=True)
        _cookie_secret_file.write_text(COOKIE_SECRET.hex() + "\n")
        _cookie_secret_file.chmod(0o600)
    except OSError as e:
        print(f"[server] could not persist cookie secret: {e!r}", flush=True)
...
<div class="card">
  ...
  <p class="footnote">Credentials are <code>ADMIN_USERNAME</code> and <code>ADMIN_PASSWORD</code>. If unset, a password is generated and persisted on /data.</p>
</div>
</body></html>"""
...
class Dashboard:
    ...
    async def _start_locked(self):
        ...
        try:
            ...
            if public_url:
                ...
                print("[dashboard] auth gate OFF — no public URL configured; "
                      "MCP OAuth redirects may point at the loopback address. "
                      "Set HERMES_DASHBOARD_PUBLIC_URL for OAuth callbacks.", flush=True)
            self._drain_task = asyncio.create_task(self._drain())
        except Exception as e:
            print(f"[dashboard] FAILED to spawn: {e!r}", flush=True)

    async def _drain(self):
        """Stream subprocess output to the container logs and a ring buffer."""
        ...
        try:
            ...
            if rc is not None and rc != 0:
                print(f"[dashboard] EXITED with code {rc} — supervising restart", flush=True)
    ...
    async def _supervise_respawn(self):
        ...
        delay = min(2 ** min(self._respawn_count - 1, 4), 15)
        await asyncio.sleep(delay)
        if self._stopping:
            return
        async with self._lifecycle_lock:
            if self._stopping or (self.proc and self.proc.returncode is None):
                return
            await self._start_locked()
